...
//    Задача 12
//        Давайте напишем программу-помощник для игры в морской бой.
//        Многие люди часто забывают отмечать клетки, в которые они стреляли,
//        не закрашивают клетки вокруг потопленных кораблей и так далее.
//        Давайте попробуем реализовать класс SeaBord, описывающий поле игры.
...
//        метод void shoot(int line, int column, String issue), принимающий
//        в себя координаты на поле (от 0 до 9, другие числа туда подать не могут)
//        и строку с исходом этого выстрела (m — miss/промах,
//        h — hit/попадание и d — destroy/уничтожение, гарантируется, что другие буквы туда подать не могут);
//        метод check(int line, int column), возвращающий . (точку),
//        если в клетку [line][column] еще не стреляли, # — если стреляли,
//        но не попали (или, например, потопили корабль, а эта клетка соседняя),
//        x — если в этой клетке было попадание.
//        Примечание:
//        Поле представляет собой двумерный массив строк 10 на 10.
//        Все клетки, в которые еще не стреляли, — это . (точки).
