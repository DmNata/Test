...
//        checkGame() — метод должен определить есть ли победитель в игре,
//        если да, то вернуть X/0 в зависимости от того, кто победил, если победителя нет,
//        то метод должен вернуть D (ничья), если игра ещё продолжается, то вернуть строку null;
//        makeMove(int x, int y), принимающий два параметра — ряд и столбик,
//        куда игрок хочет сделать ход. Если ход невозможен (клетка уже занята),
//        то метод должен вернуть строку вида {Cell x, y is already occupied}.
//        Если же ход возможен, то в поле следует поставить X или 0
//        (зависит от того, кто сейчас ходил), и вернуть строку Move completed,
//        если после этого хода игра не заканчивается. Если же этим ходом победил один из игроков,
//        то метод должен вернуть строку вида {Player X/0 won}.
//        Если же после этого хода свободных клеток не осталось, а победителя нет,
//        то метод должен вернуть Draw. Если игра уже завершена (выявлен победитель или ничья),
//        но кто-то хочет походить дальше, то метод должен вернуть Game was ended.
//        Примечание:
